...
if __name__ == "__main__":
    ...
    parser.add_argument("--data", type=int, default=None,
                        help="Value to write. If None is given, the address will be read and not written")
        ...
        for fn, f in enumerate(fmcs):
            ...
            for cs in range(args.csstart, args.csstop + 1):
                if args.data is None:
                    print("Reading address 0x%x with chip-select %d on FMC %d" % (args.addr, cs, fn))
                    readback = f.read_spi(args.addr, cs)
                else:
                    print("Writing 0x%x to address 0x%x with chip-select %d on FMC %d" % (args.data, args.addr, cs, fn))
                    readback = f.write_spi(args.addr, args.data, cs)
                #f.enable_readout(cs)
