...
class ZCU102Fengine():
    """
    A control class for LWA-SV ZCU102 F-Engine firmware.
    ...
    def is_connected(self):
        """
        :return: True if there is a working connection to a ZCU102. False otherwise.
        :rtype: bool
        """
    ...
    def _initialize_blocks(self, passive=False):
        ...
        self.noise       = noisegen.NoiseGen(self._cfpga, 'noise', n_noise=2, n_outputs=32)
        #: Control interface to Input Multiplex block
        self.input       = input.Input(self._cfpga, 'input', n_streams=32)
        #: Control interface to Coarse Delay block
        self.delay       = delay.Delay(self._cfpga, 'delay', n_streams=32)
        ...
        self.packetizer  = packetizer.Packetizer(self._cfpga, 'packetizer', n_signals=64,
    ...
    def program(self, fpgfile=None, force=False):
        """
        Program an .fpg file to a ZCU102 FPGA. If the name of the file
    ...
    def cold_start_from_config(self, config_file,
        ...
        """
        Completely configure a ZCU102 F-engine from scratch, using a configuration
        YAML file.

        :param program: If True, start by programming the ZCU102 FPGA from
            the image currently in flash. Also train the ADC-> FPGA links
            and initialize all firmware blocks. Also relock the ZCU102's
        ...
        try:
            ...
            # in 'fengines' or locally as a key within a ZCU102 host configuration
            ...
            # from counting all of the ZCU102 host configuration entries
            try:
    ...
    def cold_start(self, program=True, initialize=True, test_vectors=False,
        ...
        :param nstand_tot: Total number of stands for the entire collection of ZCU102
            boards.
        :type nstand_tot: int
